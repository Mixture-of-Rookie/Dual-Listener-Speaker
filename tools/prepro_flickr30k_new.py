...
def generate_pseudo_label_v3(data_dir):# {{{
    """Generate pseudo label for each ref in the flickr30k dataset.
    ...
    # Load glove vector
    glove_file = '/media/fuqian/568AD0AA6378D181/glove.840B.300d.txt'
    glove_dict = load_glove_feats(glove_file)
            ...
            if tmp_cls != '__background__':
                if tmp_cls in word_list:
                    ...
                    if len(noun_tokens) == 0:
                        ...
                        noun_glove_list = [np.array(glove_dict[t], dtype='float32') for t in noun_tokens
                                           if t in glove_dict]
                    if len(noun_glove_list) == 0:
                        print(phrase)
                        break

                    noun_glove_array = np.sum(np.array(noun_glove_list), axis=0).reshape((1, 300))
                    cls_glove_list = [np.array(glove_dict[t], dtype='float32') for t in tmp_cls.split(' ')]
                    cls_glove_array = np.array(cls_glove_list)
                    sim_array = np.dot(noun_glove_array, cls_glove_array.T)
                    tmp_score += np.max(sim_array)
    ...
    # Dump into disk
    with open(os.path.join(data_dir, 'flickr30k_refs_pseudo_new.pkl'), 'wb') as f:
        pickle.dump(refs, f)# }}}
...
def compute_pseudo_label_acc(data_dir):# {{{
    ...
    """
    # Load pseudo labels
    with open(os.path.join(data_dir, 'flickr30k_refs_pseudo_new.pkl'), 'rb') as f:
        refs = pickle.load(f)
...
def load_clean_entries(data_dir):# {{{
    with open(os.path.join(data_dir, 'flickr30k_clean_entries_test.pkl'), 'rb') as f:
    ...
    for entry in entries:
        ...
        attrs = entry['attrs']
        labels = entry['labels']
        phrase_ids = entry['entity_ids']
        noun_tokens = entry['noun_tokens']
        target_bboxes = entry['target_bboxes']
        detected_bboxes = entry['detected_bboxes']

        num_queries = len(query)
        assert len(query) == len(phrase_ids) == len(noun_tokens) == len(target_bboxes)
        for n in range(num_queries):
            new_query = query[n]
            new_phrase_id = phrase_ids[n]
            new_noun_tokens = noun_tokens[n]
            new_target_bbox = target_bboxes[n]

            if new_phrase_id not in new_entries:
                new_entries[new_phrase_id] = []
            new_entries[new_phrase_id] += [{
                'image_name': image + '.jpg',
                'ann_id': new_phrase_id,
                'raw_phrase': new_query,
                'gt_bbox': new_target_bbox,
                'noun_tokens': new_noun_tokens,
                'proposals': detected_bboxes,
                'labels': labels,
                'attrs': attrs,
            }]

    with open(os.path.join(data_dir, 'flickr30k_entries_test.pkl'), 'wb') as f:
        pickle.dump(new_entries, f)# }}}


def combine_entries(data_dir):# {{{
    with open(os.path.join(data_dir, 'flickr30k_entries_test.pkl'), 'rb') as f:
        entries = pickle.load(f)
    ...
    for ref in refs:
        ...
        if ann_id in entries:
            entry_list = entries[ann_id]
            for entry in entry_list:
                if raw_phrase == entry['raw_phrase']:
                    num_combined += 1
                    ref['noun_tokens'] = entry['noun_tokens']
                    break
...
def main(args):
    ...
    #load_clean_entries(data_dir)
    #combine_entries(data_dir)
    #generate_pseudo_label_v3(data_dir)
